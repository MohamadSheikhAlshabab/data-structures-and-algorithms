...
const toTitleCase = (arr) => {
    ...
    arr.forEach(element => {
        ...
        arrOfFirstLetters.push(element.charAt(0).toUpperCase() + element.slice(1));
    })
...
let biggerThanLuke = (arr) => {
    ...
    let arrOfFirstLetters = [];
        for (let i = 0; i < arr.length; i++) {
            let splitName = starWarsData[i].name.split(" ");
            let regex = (/\W+/g);
            let r = splitName[0].match(regex)
              if (splitName[0].length !== 4 && splitName[0].includes != (regex) && r == null) {
                arrOfFirstLetters.push(starWarsData[i].name);
                retunrValue = arrOfFirstLetters.join(' - ');
            }
        }
        // console.log('qwe',arr? retunrValue:"");
    return arr? retunrValue:"" ;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
